...
import re
try:
    from intent import Loki_BuyInsurance
except:
    from .intent import Loki_BuyInsurance
    ...
    if lokiRst.getStatus():
        for index, key in enumerate(inputLIST):
            for resultIndex in range(0, lokiRst.getLokiLen(index)):
                # BuyInsurance
                if lokiRst.getIntent(index, resultIndex) == "BuyInsurance":
                    resultDICT = Loki_BuyInsurance.getResult(key, lokiRst.getUtterance(index, resultIndex), lokiRst.getArgs(index, resultIndex), resultDICT)
...
def testIntent():
    # BuyInsurance
    print("[TEST] BuyInsurance")
    inputLIST = ['我想買保險','我想保旅行險','我想要幫自己投保','我想要幫自己買保險','詢問出國的旅行保險','我想要幫家人買一份保險','我想幫家人買一份醫療保單','出國時保哪一種保險比較適合','我想要了解一下關於醫療的保險','針對老人家，什麼保單比較適合','我想要了解關於終生型的醫療保險']
    testLoki(inputLIST, ['BuyInsurance'])
    print("")
